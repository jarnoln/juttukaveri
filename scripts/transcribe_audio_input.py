...
def transcribe_audio_input(language: str, age: int) -> None:
    ...
    if language.startswith("fi"):
        ...
        context = "Olet ystävällinen lastenopettaja. Keskustelet {}-vuotiaan lapsen kanssa.".format(
            age
        )
    else:
        language_code = "en-US"
        ...
        context = "You are a friendly kindergarten teacher. You are discussing with a {}-year old child.".format(
            age
        )
    ...
    while True:
        with mic as source:
            ...
            try:
                result_google = recognizer.recognize_google(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--language", help="Expected input (and output) language", default="finnish"
    )
